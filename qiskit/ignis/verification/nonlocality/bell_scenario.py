...
class BellScenario():
    ...
    @s.setter
    def s(self, s):
        assert hasattr(s, '__iter__') and not isinstance(s, str) and len(
            s) > 0, ' settings should be given as a sequence of numbers'
        sett = []
        for i in s:
            try:
                if isinstance(i, int):
                    assert not isinstance(
                        i, (bool, np.bool_)), ' setting can\'t be bool'
                else:
    ...
    @d.setter
    def d(self, d):
        assert not isinstance(d, (bool, np.bool_)) and float(
            d).is_integer() and float(d) > 1, ' d should be int > 1'
        self._d = int(float(d))
    ...
    def inputs(self, row: int) -> list:
        ...
        try:
            if isinstance(row, int):
                assert not isinstance(
                    row, (bool, np.bool_)), ' row can\'t be bool'
            else:
                assert float(row).is_integer(), ' row should be int'
                row = int(float(row))
            assert 0 <= row < self.rowsA, ' row index {0} out of range for this problem: 0 <= row < {1}'.format(
                row, self.rowsA)
        except AssertionError as aerr:
            logging.error(aerr)
        except TypeError as terr:
    ...
    def row(self, inputs: list, outputs: list) -> int:
        ...
        """
        try:
            assert hasattr(inputs, '__iter__') and len(
                inputs) == self.n, ' len(inputs) doesn\'t match n'
            ...
            assert hasattr(outputs, '__iter__') and len(
                outputs) == self.n, ' len(outputs) does not match n'
            assert all(isinstance(i, (int, np.int64)) and not isinstance(i, (bool, np.bool_))
                       and 0 <= i < self.d for i in outputs), ' outputs out of range'
        except AssertionError as exception:
            logging.exception(exception, exc_info=False)
        else:
            rid = int(''.join([(string.digits+string.ascii_lowercase)[i]
                               for i in outputs]), base=self.d)
            mid = np.inner(inputs, [np.prod(self.s[i:])
    ...
    def local_corr(self, corr: dict):
        ...
        try:
            ...
            logging.error(' corr should be a dictionary {row: non0ind(row)}')
        else:
    ...
    def get_local_corr(self, row: int) -> list:
        ...
        try:
            ...
            logging.error(terr) #' TypeError: {}'.format(terr))
    ...
    def nonredundant(self, nonredundant: dict):
        try:
            assert isinstance(nonredundant, dict)
        except AssertionError:
            logging.error(
                ' nonredundant should be a dictionary {row: isnonredundant(row)}')
        else:
    ...
    def get_nonredundant(self, row: int) -> bool:
        ...
        try:
            ...
            logging.error(err) #' TypeError: {}' % str(err))
