...
class BellScenario():
    ...
    def __init__(self, sett, d = 2):
        ...
        Args:
            sett: sequence that specifies the number of possible measurement 
                settings per system; len(sett) specifies the number of subsystems.
    ...
    @property
    def s(self) -> list:
        """ Return possible measurement settings per subsystem."""
        return self._s
    ...
    @property
    def d(self) -> int:
        """ Return the number of possible outcomes, equal to the
            local Hilbert space dimension and the same for each subsystem."""
        return self._d
    ...
    @property
    def rowsA(self) -> int:
        """ Return the dimension of the space of behaviours 
            (i.e. number of All rows in LP problem)."""
        if self.s:
    ...
    @property
    def rowsB(self) -> int:
        """ Return the dimension of Bell polytope 
            (i.e. number of rows in reduced LP problem). """
        return int(np.prod(np.array(self.s)*(self.d-1)+1, dtype=np.float64) - 1)

    @property
    def cols(self) -> int:
        """ Return the length of joint probabilities in LHV model 
            (i.e. number of columns in LP problem). """
    ...
    def row(self, inputs: list, outputs: list) -> int:
        """ Return an index of a constraint (row) in an optimization problem, 
    ...
    def local_corr(self) -> dict:
        """ Return the dictionary mapping row indices to lists of nonzero 
    ...
    def get_local_corr(self, row: int) -> list:
        """ Return a list of indices of nonzero elements in Bell's local model 
    ...
    def _non0ind(self, row: int) -> list:
        """ Determine indices of nonzero elements in Bell's local model of 
    ...
    def nonredundant(self) -> dict:
        """ Return a dict mapping row indices to bool indicating if a given row 
    ...
    def get_nonredundant(self, row: int) -> bool:
        """ Return a bool indicating if a given row is nonredundant (True) 
