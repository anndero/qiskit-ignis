...
class BLocFitter(BellScenario):
    ...
    #--------------------------------------------------------------------------------------------
    def __init__(self, sett, backend_results = [], statevector = [], unitary_seq = [] ):
        ...
        self._calc_quant_corr = {} # calculated quantum correlations based on rho and unitary_seq
    ...
    def add_results(self, backend_results, clear = False ):
        ...
        try: 
            ...
            ' backend results should be a list of length {}'.format(np.prod(self.s))
            ...
            if backend_results == []: return
            else: logging.error(aerr)
        else:
            if clear: self._results = [] 
            ...
            for results in self._results:
                ...
                for result in results:
                    counts.extend([v for k, v in sorted(result.get_counts().items())])
    ...
    def rho(self, state):
        ...
        elif self._test_rho(state): self._rho = state.astype('complex')
        else: self._rho = None
    @staticmethod
    def _test_rho(rho):
        """ tests if a matrix is a correct density matrix """
        if np.array(rho).ndim != 2: return False # matrix
        if len(set(rho.shape)) != 1: return False # square matrix
        if not np.allclose(np.trace(rho), 1.0): return False # trace = 1
        if not np.allclose(rho, np.transpose(np.conj(rho))): return False # Hermitian
        return np.all(v>=0 or np.isclose(v,0.0,atol=1e-15) for v in np.linalg.eigvals(rho).real) # positive semidefinite
    ...
    def u(self, u):
        """ setter checks lists lengths, but assumes that lists' elements are unitary matrices;
            also clears """
        if len(u) == self.n and all(len(u[i]) == self.s[i] for i in range(self.n)): self._u = u  
        else: self._u = None
    ...
    def get_quantum_corr(self, source: str, row: int) -> np.float64:
        ...
        if source.lower() == 'meas':
            ...
            logging.error(' source of quantum correlations not recognized, can be \'meas\' or \'calc\'')
    ...
    def meas_quant_corr(self, corr: list):
        if np.isclose(sum(corr),np.prod(self.s)): self._meas_quant_corr = corr
        else: self._meas_quant_corr = []
    ...
    def calc_quant_corr(self, corr: dict):
        ...
        except AssertionError: logging.error(' corr should be a dictionary {row: quantum_prob(row)}')
        else: self._calc_quant_corr = corr
    ...
    def new_LP(self, LP_type):
        ...
        if LP_type.lower() == 'feasibility':
            self.LP.variables.add(ub = np.ones(self.cols), lb = np.zeros(self.cols))
        ...
        else: logging.error(' Invalid LP_type. Should be \'feasibility\' or \'optimization\'. Objective hasn\'t been set.')      
        self.LP.objective.set_sense(self.LP.objective.sense.maximize)
        
        # summation constraint
        self.LP.linear_constraints.add(
            lin_expr = [cplex.SparsePair(ind = range(self.cols), val = np.ones(self.cols))], 
    ...
    def populate_LP(self, rows, LP_type, source):  
        ...
        try:
            if isinstance(rows, (int,np.int64)) and not(isinstance(rows, (bool,np.bool_))): rows=[rows]
        ...
        try: 
            ...
            else: logging.error(' Invalid LP_type. Constraints hasn\'t been added.')     
        except TypeError: logging.error(' Invalid rows. Constraints hasn\'t been added.') 
    ...
    def test(self, source = 'meas', rows = None, nosignaling = False, # basic args to build LP
        ...
        """
        try:
            assert source.lower() in ['meas','calc'], ' source should either \'meas\' or \'calc\''
            assert nosignaling in [True, False],' nosignaling should be True or False' 
            
            if rows == None: Rows = range(self.rowsA)
            else: 
                assert hasattr(rows,'__iter__') and not(isinstance(rows,str)),' rows should be iterable and not str'
                Rows = set(rows) # remove duplicates       
            assert len(Rows) < sys.maxsize, ' problem is too big - will be handle differently'
            Rows = list(Rows)
            
            if max_size == None: # solve the full LP at the end
                if nosignaling == True: max_size = min(self.rowsB,len(Rows)) # full LP can be smaller than max_size because of redundancy checking    
                else: max_size = len(Rows)
            else:
                assert float(max_size).is_integer() and not isinstance(max_size,(bool,np.bool_)),' max_size should be int'
            
            subset = []
            if min_size == None: min_size = max_size # solve the LP without iteration
            elif hasattr(min_size,'__iter__') and not(isinstance(min_size,str)): # custom initial subset
                subset = list(set(min_size.copy()))
                min_size = len(subset)  
            else:
                assert int(min_size)==min_size and not isinstance(min_size, (bool,np.bool_)),' min_size should be int'

            assert 1 <= min_size <= max_size <= len(Rows), ' wroung bounds on subset size'
            
            if step == None: step = 1
            assert int(step)==step and not isinstance(step,(bool,np.bool_)), ' step should be int' 
        ...
        for size in it.chain(range(min_size, max_size, step), [max_size]):
            ...
            NL = True if status=='infeasible' else (False if status=='optimal' else status)
            if progressbar == True:
